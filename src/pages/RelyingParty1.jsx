...
const RelyingParty1 = () => {
  ...
  const handlePaste = async () => {
    try {
      ...
      // toast.error("Invalid clipboard data!", { position: "top-center" });
    }
  };

  // Keyboard shortcut for Ctrl+R / Cmd+R
  useEffect(() => {
    const handleKeyDown = (e) => {
      const isMac = navigator.platform.toUpperCase().indexOf("MAC") >= 0;
      if (
        (isMac && e.metaKey && e.key === "r") ||
  ...
  return (
    <div className="p-8 max-w-md mx-auto relative">
      <h2 className="text-xl font-bold mb-4">Relying Party 1 - Online Store 1</h2>

      {!submitted ? (
        <>
          <button
            className="absolute inset-y-0 right-[0px] flex items-center text-gray-600 hover:text-gray-900"
            onClick={handlePaste}
          >
            <FiClipboard size={18} />
          </button>
